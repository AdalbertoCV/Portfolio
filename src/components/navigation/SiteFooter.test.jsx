...
test('the footer explores the same six pages as the navbar, in order', () => {
  render(
    <I18nProvider>
      <MemoryRouter>
        <SiteFooter />
  ...
  const labels = [en.nav.about, en.nav.experience, en.nav.ventures, en.nav.projects, en.nav.stack, en.nav.contact];
  const links = labels.map((label) => screen.getByRole('link', { name: label }));
  expect(links.map((link) => link.getAttribute('href'))).toEqual([
    '/', '/experience', '/ventures', '/projects', '/stack', '/contact',
  ]);
